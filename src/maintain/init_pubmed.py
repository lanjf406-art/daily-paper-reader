...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch PubMed records and sync them to Supabase.")
    ...
    if raw_path:
        ...
        raw_path = os.path.join(project_root, "archive", date_str, "raw", f"pubmed_papers_{date_str}.json")

    if not args.skip_fetch:
        fetch_cmd = [
            python,
            os.path.join(SCRIPT_DIR, "fetchers", "fetch_pubmed.py"),
            "--days",
        ...
        if str(args.query or "").strip():
            fetch_cmd += ["--query", str(args.query).strip()]
        if str(args.email or "").strip():
            fetch_cmd += ["--email", str(args.email).strip()]
        if args.ignore_seen:
            fetch_cmd.append("--ignore-seen")
        run_step("Step 1 - fetch PubMed", fetch_cmd)
    else:
        print(f"[INFO] Step 1 skipped, reuse raw file: {raw_path}", flush=True)
    ...
    run_step("Step 2 - sync PubMed to Supabase", sync_cmd)
